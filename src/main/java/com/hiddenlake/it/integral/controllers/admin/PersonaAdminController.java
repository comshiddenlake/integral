...
public class PersonaAdminController {
	...
	public ResponseEntity<?> responseGet(/* @CurrentUser UserPrincipal currentUser */) throws EntityNotFoundException {
		logger.info("GET | Asistencias FindAll: " + personaServiceImpl.findAll());
	...
	public ResponseEntity<?> addPersona(@RequestBody Persona persona) {
		logger.info("POST - Admin | Persona addPersona: " + persona);
	...
	public ResponseEntity<?> updateAsistenciaTipo(@RequestBody Persona persona) {
		try {
			logger.info("ADMIN PUT | User insertUser:" + persona);
